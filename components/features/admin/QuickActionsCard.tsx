...
export default function QuickActionsCard() {
  ...
  return (
    <div className="stat-card animate-fade-in">
      <div className="mb-4">
        <h3 className="font-serif text-lg font-semibold text-foreground">Ações Rápidas</h3>
        <p className="text-sm text-muted-foreground">Crie novo conteúdo rapidamente</p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {quickActions.map((action) => (
          <Link key={action.label} href={action.isAvailable ? action.href : "#"}>
            <Button
              variant="outline"
