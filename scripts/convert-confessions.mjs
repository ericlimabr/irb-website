...
/** Strip note markers, emphasis underscores; collapse whitespace runs. */
function cleanInline(text) {
  return text
    .replace(/<sup>[^<]*<\/sup>/g, "") // note markers
    .replace(/_([^_]+)_/g, "$1") // italic emphasis → plain
    .replace(/\s+/g, " ")
    .trim()
}
...
function parseDort() {
  ...
  for (const line of lines) {
    ...
    if (/^##\s+REJEI/i.test(line) || /^#\s+CONCLUS/i.test(line)) {
      flush()
      cur = null
      inRejection = true
      continue
...
export const DORT_ARTICLES: DortArticle[] = ${json(dort)}
`,
)
...
console.log(`Dort:       ${dort.length} articles`)
console.log(
